...
begin
  ...
  RectFrom := IntertelecomForm.BoundsRect;
  GotRectTo := False;
  ...
  if Minimizing then
    DrawAnimatedRects(IntertelecomForm.Handle, IDANI_CAPTION, RectFrom, RectTo)
  else
    DrawAnimatedRects(IntertelecomForm.Handle, IDANI_CAPTION, RectTo, RectFrom);
...
begin
{$IFDEF DELPHI_6_UP}
  BlendValue := IntertelecomForm.AlphaBlendValue;
{$ENDIF}
...
{$IFDEF DELPHI_6_UP}
  if (BlendValue >= 0) and (BlendValue <= 255) then
    IntertelecomForm.AlphaBlendValue := BlendValue;
{$ENDIF}
end;
...
begin
  SetWindowPos(IntertelecomForm.Handle, 0, X, Y, W, H, SWP_NOZORDER);
end;
